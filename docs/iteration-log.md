...
two exploration passes after them, run in parallel from the same brief. Each
critique pass: build, screenshot every page at 1440 / 768 / 390 in both themes,
look at the screenshots, name the single largest gap, fix that one gap, run the
whole Playwright suite.

The exploration passes ran the other way round — prototypes first, then a
choice, then a build — and are written up at the end. **Pass A** was type-led;
**pass B** was told to find the idea somewhere other than the type. They reached
the same conclusion from opposite directions and each took its own headline idea
back out, which is recorded under "Creativity pass B, third look".
